import { click, type, waitForPageLoadState } from '@ActionUtils';
import {
...
const autoSearchSuggestions = () =>
  getLocatorInFrame(
    ...
    `//*[@class='google-suggestions']//*[contains(@qaid,'store-locator__search__suggestions__')]`,
  );
const storeResults = () =>
  getLocatorInFrame(
    `#lcly-embedded-iframe-inner-0`,
    `//*[@id='conversion-locations']//*[@class='section-title dl-store-name']`,
  ).nth(0);
...
export async function verifyAutoSearchResultsDisplayed() {
  await expectElementToBeVisible(autoSearchSuggestions(), {
    timeout: STANDARD_TIMEOUT,
  });
}
...
export async function clickAutoSearchResult(resultNumber: number) {
  await click(
    getLocatorInFrame(
      `#lcly-embedded-iframe-inner-0`,
      `//*[@qaid='store-locator__search__suggestions']//li[${resultNumber}]`,
    ),
  );
